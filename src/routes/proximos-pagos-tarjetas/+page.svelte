<script lang="ts">
	...
	import type { ProximoPagoTarjeta } from '$lib/tipos';

	let loading = $state(true);
	let error = $state('');
	let proximosPagos: ProximoPagoTarjeta[] = $state([]);

	async function loadProximosPagos() {
		try {
			const response = await apiGet('/api/proximos-pagos-tarjetas');
	...
	<div class="min-h-screen bg-gray-50 py-8">
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
			<div class="mb-8">
				...
				<p class="mt-2 text-gray-600">Todos los pagos pendientes de tus tarjetas de crédito</p>
			</div>
			...
			{:else}
						...
						<div class="flex items-center">
							...
							<div class="ml-5">
								...
								<p class="text-2xl font-bold text-gray-900">
									{formatCurrency(
										proximosPagos.reduce((sum, p) => sum + parseFloat(p.monto_pago ?? '0'), 0)
									)}
								</p>
							</div>
						</div>
							...
							<div class="ml-5">
								...
								<p class="text-2xl font-bold text-gray-900">
									{proximosPagos.filter(
										(p) =>
											getDiasRestantes(p.fecha_limite_pago) <= 7 &&
											getDiasRestantes(p.fecha_limite_pago) >= 0
									).length}
								</p>
							</div>
						</div>
				...
				<!-- Lista de pagos -->
				<div class="space-y-4">
					{#each proximosPagos as pago (pago.id_tarjeta)}
									...
									<div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
										<div>
											...
											<p class="font-medium text-gray-900">
												{formatDate(pago.fecha_corte_anterior, {
													day: '2-digit',
													month: '2-digit'
												})}
												al
