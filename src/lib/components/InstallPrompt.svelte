<script lang="ts">
	...
	import { estaInstalada, esIOS, type EventoInstalacion } from '$lib/utils/navegador';

	let showPrompt = $state(false);
	let deferredPrompt: EventoInstalacion | null = null;
	...
	onMount(() => {
		...
		const isStandalone = estaInstalada() || document.referrer.includes('android-app://');
		...
		window.addEventListener('beforeinstallprompt', (e: Event) => {
			e.preventDefault();
			deferredPrompt = e as EventoInstalacion;
			setTimeout(() => {
				showPrompt = true;
	...
	>
		...
		>
			<div class="flex justify-between items-start mb-4">
				<h3 id="ios-install-title" class="text-xl font-bold text-gray-900">Cómo Instalar en iOS</h3>
				<button onclick={dismiss} class="text-gray-400 hover:text-gray-600" aria-label="Cerrar">
